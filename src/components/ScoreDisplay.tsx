...
export const ScoreDisplay = ({ score, totalQuestions, streak, bestStreak, playerName }: ScoreDisplayProps) => {
  ...
  return (
    <div className="flex flex-col items-center gap-2">
      {playerName && (
        <div className="text-lg font-semibold text-primary mb-2">Player: <span className="text-foreground">{playerName}</span></div>
      )}
      <div className="flex flex-wrap justify-center gap-4 md:gap-6">
        ...
        <div className="flex items-center gap-2 px-4 py-2 rounded-full card-gradient border border-border">
          ...
          <span className={`font-bold ${streak > 0 ? 'text-accent' : 'text-muted-foreground'}`}>{streak}</span>
